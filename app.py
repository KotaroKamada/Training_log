            ...
            if st.session_state.selected_exercise_idx == idx:
                ...
                with st.expander(f"記録入力: {exercise_title}", expanded=True):
                    ...
                    for set_num in range(actual_sets):
                        ...
                        # セット1以降でセッション状態から値を取得
                        use_copied_values = set_num > 0 and f"copy_unit_{idx}" in st.session_state
                        
                        with col1:
                            unit_default = 0
                            if use_copied_values:
                                units = ["kg", "%", "体重", "その他"]
                                saved_unit = st.session_state.get(f"copy_unit_{idx}", "kg")
                                unit_default = units.index(saved_unit) if saved_unit in units else 0
                        ...
                        with col2:
                            if unit == "その他":
                                load_default = ""
                                if use_copied_values:
                                    load_default = st.session_state.get(f"copy_load_text_{idx}", "")
                                ...
                                load_default = 0.0
                                if use_copied_values:
                                    load_default = st.session_state.get(f"copy_load_val_{idx}", 0.0)
                        ...
                        with col3:
                            rep_default = 1
                            if use_copied_values:
                                rep_default = st.session_state.get(f"copy_rep_{idx}", 1)
