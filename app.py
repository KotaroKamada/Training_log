...
def init_google_sheets():
    try:
        ...
        st.sidebar.error(f"❌ Google Sheets 接続失敗: {e}")
        return None

# Google Sheets からデータを読み込み（改良版）
def load_data_from_sheets(spreadsheet):
    try:
        ...
        if data:
            ...
            st.sidebar.info(f"📊 Google Sheets読み込み: {len(df)}件")
            return df
        else:
        ...
        st.sidebar.warning("⚠️ 'TrainingLog'シートが見つかりません。作成中...")
        # シートが存在しない場合は作成
...
def save_data_to_sheets(spreadsheet, new_data):
    try:
        ...
        st.sidebar.error(f"❌ Google Sheets保存エラー: {e}")
        return False
