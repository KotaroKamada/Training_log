...
def init_google_sheets():
    try:
        ...
        st.sidebar.success("✅ Google Sheets 接続成功")
        return spreadsheet
        
    except Exception as e:
        st.sidebar.error(f"❌ Google Sheets 接続失敗: {str(e)[:50]}...")
        return None
...
def wait_for_api_limit():
    """API制限に引っかかった場合の待機"""
    import time
    st.sidebar.warning("⏳ API制限のため60秒待機中...")
    time.sleep(60)
    st.sidebar.info("✅ 待機完了、再試行します")

# データクリーニング関数を追加
def clean_dataframe_for_display(df):
    """
    DataFrameを表示用にクリーンアップ（Arrow変換エラーを防ぐ）
    """
    if df.empty:
        return df
    ...
    # 数値列として扱うべき列を文字列に統一
    numeric_columns = ['回数', 'set', '総負荷量', 'レップ数', 'セット数']
    
    for col in numeric_columns:
        if col in df_cleaned.columns:
            # 数値以外の文字列が含まれている場合は文字列として扱う
...
# Google Sheets からデータを読み込み（キャッシュ付き改良版）
@st.cache_data(ttl=60)  # 60秒間キャッシュ
def load_data_from_sheets_cached(spreadsheet_id):
    try:
        if not spreadsheet:
            return pd.DataFrame(columns=["日付", "プログラム名", "名前", "エクササイズ名", "カテゴリー", "set", "負荷", "回数", "総負荷量"])
        ...
        if data:
            ...
            # 数値列を適切な形式に変換（エラーを無視）
            if '回数' in df.columns:
                df['回数'] = pd.to_numeric(df['回数'], errors='coerce').fillna(0)
            if '総負荷量' in df.columns:
                df['総負荷量'] = pd.to_numeric(df['総負荷量'], errors='coerce').fillna(0)
        ...
        # シートが存在しない場合は作成
        try:
            worksheet = spreadsheet.add_worksheet(title="TrainingLog", rows="1000", cols="9")
            # ヘッダーを設定（カテゴリー列を追加）
            worksheet.append_row(["日付", "プログラム名", "名前", "エクササイズ名", "カテゴリー", "set", "負荷", "回数", "総負荷量"])
        except:
            pass
        return pd.DataFrame(columns=["日付", "プログラム名", "名前", "エクササイズ名", "カテゴリー", "set", "負荷", "回数", "総負荷量"])
    except Exception as e:
        st.sidebar.error(f"❌ API制限または接続エラー: {str(e)[:100]}...")
        # フォールバック: 空のDataFrameを返す
        return pd.DataFrame(columns=["日付", "プログラム名", "名前", "エクササイズ名", "カテゴリー", "set", "負荷", "回数", "総負荷量"])

def load_data_from_sheets(spreadsheet):
    """キャッシュ付きでGoogle Sheetsからデータを読み込み"""
    if spreadsheet:
        spreadsheet_id = st.secrets["sheets"]["spreadsheet_id"] if "sheets" in st.secrets else "unknown"
        df = load_data_from_sheets_cached(spreadsheet_id)
        st.sidebar.info(f"📊 Google Sheets読み込み: {len(df)}件 (キャッシュ使用)")
        return df
    else:
        return pd.DataFrame(columns=["日付", "プログラム名", "名前", "エクササイズ名", "カテゴリー", "set", "負荷", "回数", "総負荷量"])

# 特定選手の特定エクササイズの履歴を取得する関数
def get_exercise_history(df, player_name, exercise_name, limit=5):
    """
    指定された選手の指定されたエクササイズの履歴を取得
    """
...
# Google Sheets にデータを保存（API制限対応版）
def save_data_to_sheets(spreadsheet, new_data):
    try:
        if not spreadsheet:
            return False
        ...
        # バッチ処理で一度に全データを追加（API呼び出し回数を削減）
        rows_to_add = []
        for _, row in new_data.iterrows():
            row_data = [
                row['日付'].strftime('%Y-%m-%d') if pd.notna(row['日付']) else '',
                str(row['プログラム名']),
                str(row['名前']),
                str(row['エクササイズ名']),
                str(row['カテゴリー']) if 'カテゴリー' in row else '',  # カテゴリー列を追加
                str(row['set']),
                str(row['負荷']),
                str(row['回数']),
                float(row['総負荷量']) if pd.notna(row['総負荷量']) else 0
            ]
        ...
        st.sidebar.success(f"✅ Google Sheets保存成功: {len(new_data)}件")
        
        # キャッシュをクリアして最新データを表示
        load_data_from_sheets_cached.clear()
        
        return True
        
    except Exception as e:
        if "429" in str(e) or "Quota exceeded" in str(e):
            st.sidebar.error("❌ API制限に達しました。しばらく待ってから再試行してください。")
        else:
            st.sidebar.error(f"❌ Google Sheets保存エラー: {str(e)[:50]}...")
        return False

# Google Sheets 接続を初期化
...
def load_program_file():
    try:
        if os.path.exists(PROGRAM_FILE):
            ...
            # Category列が存在しない場合は空文字で追加
            if 'Category' not in df.columns:
                df['Category'] = ''
            
            return df
        else:
            # サンプルファイルを作成（カテゴリー列を追加）
...
# ログファイルの読み込み（Google Sheets優先）
def load_training_log():
    if spreadsheet:
        # Google Sheets からデータを読み込み
        return load_data_from_sheets(spreadsheet)
    else:
        # フォールバック: ローカルExcelファイル
        if os.path.exists(LOG_FILE):
            try:
                ...
                if 'カテゴリー' not in df.columns:
                    df['カテゴリー'] = ''
                st.sidebar.info(f"📊 ローカルファイル読み込み: {len(df)}件")
                return df
            except Exception as e:
                st.sidebar.error(f"❌ ローカルファイル読み込みエラー: {e}")
                return pd.DataFrame(columns=["日付", "プログラム名", "名前", "エクササイズ名", "カテゴリー", "set", "負荷", "回数", "総負荷量"])
        else:
            return pd.DataFrame(columns=["日付", "プログラム名", "名前", "エクササイズ名", "カテゴリー", "set", "負荷", "回数", "総負荷量"])
...
# 新しいログ保存関数（Google Sheets優先）
def save_training_log_formatted(player_name, program_name, exercise_name, exercise_category, sets_data, date=None):
    if date is None:
    ...
    for set_data in sets_data:
        ...
        new_row = {
            ...
            'カテゴリー': exercise_category,  # カテゴリー列を追加
    ...
    # Google Sheets に保存を試行
    if spreadsheet:
        success = save_data_to_sheets(spreadsheet, new_df)
        if success:
            return len(new_rows)
    
    # フォールバック: ローカルExcelファイルに保存
    try:
        ...
        if len(existing_df) > 0:
            # 列名を統一
            expected_columns = ['日付', 'プログラム名', '名前', 'エクササイズ名', 'カテゴリー', 'set', '負荷', '回数', '総負荷量']
        ...
        st.sidebar.warning(f"⚠️ ローカルファイルに保存: {LOG_FILE} ({len(new_rows)}件追加)")
        return len(new_rows)
...
{exercise_details}"""
                ...
                with st.expander(f"記録入力: {exercise_title}", expanded=True):
                    ...
                    if exercise.get('Category') and exercise['Category'] != '':
                        ...
                        ">
                            <div style="
                                ...
                                font-size: 14px;
                            ">
                                Category: {get_category_display(exercise['Category'])}
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                    
                    # Google Sheetsから前回のトレーニングログを表示（改良版）
                    ...
                    if len(log_df) > 0 and player_name:
                        ...
                        if len(exercise_history) > 0:
                            ...
                            if stats:
                                ...
                                ">
                                    ...
                                    ">
                                        ...
                                        ">
                                            <div style="color: #666; font-size: 11px; font-weight: 600; margin-bottom: 4px;">負荷</div>
                                            <div style="color: #1976d2; font-size: 16px; font-weight: 700;">{stats['latest_load']}</div>
                                        </div>
                                        <div style="
                                            background: rgba(255, 255, 255, 0.8);
                                            padding: 10px;
                                            ...
                                            border: 1px solid rgba(25, 118, 210, 0.2);
                                        ">
                                            <div style="color: #666; font-size: 11px; font-weight: 600; margin-bottom: 4px;">回数</div>
                                            <div style="color: #1976d2; font-size: 16px; font-weight: 700;">{stats['latest_reps']}</div>
                                        </div>
                                        <div style="
                                            background: rgba(255, 255, 255, 0.8);
                                            padding: 10px;
                                            ...
                                            border: 1px solid rgba(25, 118, 210, 0.2);
                                        ">
                                            <div style="color: #666; font-size: 11px; font-weight: 600; margin-bottom: 4px;">総負荷</div>
                                            <div style="color: #1976d2; font-size: 16px; font-weight: 700;">{stats['latest_total_load']:.1f}kg</div>
                                        </div>
                                        <div style="
                                            background: rgba(255, 255, 255, 0.8);
                                            padding: 10px;
                                            ...
                                            border: 1px solid rgba(25, 118, 210, 0.2);
                                        ">
                                            <div style="color: #666; font-size: 11px; font-weight: 600; margin-bottom: 4px;">実施回数</div>
                                            <div style="color: #1976d2; font-size: 16px; font-weight: 700;">{stats['total_sessions']}回</div>
                                        </div>
                                    </div>
                                </div>
                                """, unsafe_allow_html=True)
                                ...
                                if len(exercise_history) > 1:
                                    ...
                                    # 履歴をテーブル形式で表示
                                    for i, (_, log) in enumerate(exercise_history.iterrows()):
                                        if i < 5:  # 最大5件
                                            date_str = pd.to_datetime(log['日付']).strftime('%m/%d') if pd.notna(log['日付']) else '-'
                                            bg_color = "rgba(25, 118, 210, 0.05)" if i == 0 else "rgba(248, 249, 250, 0.5)"
                                            border_color = "rgba(25, 118, 210, 0.2)" if i == 0 else "rgba(0, 0, 0, 0.1)"
                                            ...
                                            <div style="
                                                ...
                                                font-size: 12px;
                                            ">
                                                <div style="font-weight: 600; color: #666;">{date_str}</div>
                                                <div style="color: #333;">Set {log.get('set', '-')}</div>
                                                <div style="color: #333;">{log.get('負荷', '-')}</div>
                                                <div style="color: #333;">{log.get('回数', '-')}回</div>
                                                <div style="color: #666; font-weight: 600;">{log.get('総負荷量', 0):.1f}kg</div>
                                            </div>
                                            """, unsafe_allow_html=True)
                                ...
                                if len(exercise_history) >= 2:
                                    # 前回と前々回の比較
                                    current_total = stats['latest_total_load']
                                    previous_total = exercise_history.iloc[1]['総負荷量'] if len(exercise_history) > 1 else 0
                                    
                                    if current_total > previous_total:
                                        trend_icon = "📈"
                                        trend_text = f"前回より{current_total - previous_total:.1f}kg向上"
                                        trend_color = "#4caf50"
                                    elif current_total < previous_total:
                                        trend_icon = "📉"
                                        trend_text = f"前回より{previous_total - current_total:.1f}kg減少"
                                        trend_color = "#ff9800"
                                    else:
                                        trend_icon = "➡️"
                                        trend_text = "前回と同じ"
                            ...
                            st.markdown("""
                            <div style="
                                background: linear-gradient(135deg, rgba(96, 125, 139, 0.1) 0%, rgba(120, 144, 156, 0.1) 100%);
                            ...
                            ">
                                ...
                                <div style="
                                    color: #78909c;
                                    font-size: 13px;
                                ">このエクササイズは初めてです。頑張って記録を作りましょう！</div>
                            </div>
                            """, unsafe_allow_html=True)
                        ...
                        <div style="
                            ...
                            text-align: center;
                        ">
                            <div style="color: #f57c00; font-size: 14px; font-weight: 600;">
                                ⚠️ 選手名を入力すると前回のデータが表示されます
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                    ...
                    for set_num in range(actual_sets):
                        ...
                        with col2:
                            if unit == "その他":
                                ...
                                load_default_str = "0.0"
                                if f"copy_load_val_{idx}" in st.session_state and set_num > 0:
                                    load_default_str = str(st.session_state[f"copy_load_val_{idx}"])
                                
                                load_input = st.text_input(
                                    "値",
                                    value=load_default_str,
                                ...
                                # 数値検証
                                try:
                                    load_value = float(load_input) if load_input else 0.0
                                    if load_value < 0:
                                        load_value = 0.0
                                except ValueError:
                                    load_value = 0.0
                                    st.error(f"Set {set_num + 1}: 数値を入力してください")
                        ...
                        with col3:
                            ...
                            if f"copy_rep_{idx}" in st.session_state and set_num > 0:
                                rep_default_str = str(st.session_state[f"copy_rep_{idx}"])
                            
                            rep_input = st.text_input(
                                "レップ数", 
                                value=rep_default_str,
                            ...
                            # 数値検証
                            try:
                                set_rep = int(float(rep_input)) if rep_input else 1
                                if set_rep < 0:
                                    set_rep = 1
                            except ValueError:
                                set_rep = 1
                                st.error(f"Set {set_num + 1}: 整数を入力してください")
                        ...
                        if st.button(f"{exercise['Exercise']} 完了", key=f"complete_{idx}", type="primary", use_container_width=True):
                            if not player_name:
                                ...
                                if saved_sets > 0:
                                    ...
                                    st.error("❌ データの保存に失敗しました。管理者に連絡してください。")
        ...
        with st.expander(f"{program}", expanded=len(filtered_programs) <= 3):
            ...
            if len(warmup_exercises) > 0:
                ...
                for _, warmup in warmup_exercises.iterrows():
                    ...
                    if detail_text:
                        st.markdown(f"• {exercise_type}**{warmup['Exercise']}** ({type_name}){category_display} - {detail_text}", unsafe_allow_html=True)
                    else:
                        st.markdown(f"• {exercise_type}**{warmup['Exercise']}** ({type_name}){category_display}", unsafe_allow_html=True)
            ...
            if len(main_exercises) > 0:
                ...
                # 表示用にデータを整形
                if 'Point' in main_exercises.columns and 'Category' in main_exercises.columns:
                    display_df = main_exercises[['No', 'Exercise', 'Category', 'set', 'load', 'rep', 'Point']].copy()
                    display_df.columns = ['No.', 'エクササイズ', 'カテゴリー', 'セット数', '負荷', 'レップ数', 'ポイント']
                elif 'Category' in main_exercises.columns:
                    display_df = main_exercises[['No', 'Exercise', 'Category', 'set', 'load', 'rep']].copy() if 'No' in main_exercises.columns else main_exercises[['Exercise', 'Category', 'set', 'load', 'rep']].copy()
                    if 'No' in main_exercises.columns:
                        display_df.columns = ['No.', 'エクササイズ', 'カテゴリー', 'セット数', '負荷', 'レップ数']
                    else:
                        display_df.columns = ['エクササイズ', 'カテゴリー', 'セット数', '負荷', 'レップ数']
                elif 'Point' in main_exercises.columns:
                ...
                # エクササイズ名を太文字にする（**マークを削除）
                # display_df['エクササイズ'] = display_df['エクササイズ'].apply(lambda x: f"**{x}**")
                
                # カテゴリー表示を整形
                if 'カテゴリー' in display_df.columns:
                    display_df['カテゴリー'] = display_df['カテゴリー'].apply(lambda x: x if x != '' and pd.notna(x) else '-')
                
                # インデックスを1から始まる連番に変更
                display_df.index = range(1, len(display_df) + 1)
                
                # データクリーニングを適用
                display_df_cleaned = clean_dataframe_for_display(display_df)
    ...
    with col3:
        ...
        if 'カテゴリー' in log_df.columns:
            available_categories = ["すべて"] + [cat for cat in sorted(log_df['カテゴリー'].unique().tolist()) if cat != '' and pd.notna(cat)]
            selected_category = st.selectbox("カテゴリー", available_categories)
        else:
            selected_category = "すべて"
            st.selectbox("カテゴリー", ["すべて"], disabled=True)
    ...
    # カテゴリーでフィルタ
    if selected_category != "すべて" and 'カテゴリー' in filtered_df.columns:
        filtered_df = filtered_df[filtered_df['カテゴリー'] == selected_category]
    ...
    if len(filtered_df) > 0:
        # データテーブルで表示（クリーニング適用）
        display_columns = ['日付', 'プログラム名', '名前', 'エクササイズ名', 'カテゴリー', 'set', '負荷', '回数', '総負荷量']
        ...
        if len(filtered_df) > 0:
            ...
            with col_stat5:
                if 'カテゴリー' in filtered_df.columns:
                    unique_categories = len([cat for cat in filtered_df['カテゴリー'].unique() if cat != '' and pd.notna(cat)])
                    st.metric("実施カテゴリー数", unique_categories)
        
        # カテゴリー別統計
        if 'カテゴリー' in filtered_df.columns and len(filtered_df) > 0:
            st.markdown("### カテゴリー別統計")
            category_stats = filtered_df[filtered_df['カテゴリー'] != ''].groupby('カテゴリー').agg({
                'set': 'count',
                '総負荷量': 'sum'
            }).round(1)
            category_stats.columns = ['セット数', '総負荷量(kg)']
            ...
            if len(category_stats) > 0:
                ...
                if len(category_stats) > 1:
                    st.markdown("#### カテゴリー別セット数")
                    st.bar_chart(category_stats['セット数'])
    ...
    if spreadsheet:
        ...
        try:
            worksheet = spreadsheet.worksheet("TrainingLog")
            data = worksheet.get_all_records()
            st.info(f"📊 Google Sheets データ: {len(data)}件")
            
            # Google Sheets のURL表示
            sheets_url = f"https://docs.google.com/spreadsheets/d/{st.secrets['sheets']['spreadsheet_id']}/edit"
            st.markdown(f"🔗 [Google Sheets を開く]({sheets_url})")
            
        except Exception as e:
            st.warning(f"⚠️ Google Sheets情報取得エラー: {e}")
    else:
        st.error("❌ Google Sheets 未接続 - ローカルファイルモードで動作")
    ...
    with col_file1:
        ...
        if spreadsheet:
            try:
                worksheet = spreadsheet.worksheet("TrainingLog")
                data = worksheet.get_all_records()
                st.success(f"Google Sheets: {len(data)}件のログ")
                
                # Google Sheets データのバックアップダウンロード
                if data:
                    df = pd.DataFrame(data)
                    csv = df.to_csv(index=False, encoding='utf-8-sig')
                    st.download_button(
                        "📥 Google Sheets データをダウンロード",
                        csv,
                        f"google_sheets_backup_{datetime.today().strftime('%Y%m%d')}.csv",
                        "text/csv"
                    )
        ...
        if os.path.exists(LOG_FILE):
            log_df = load_training_log()
            st.info(f"ローカルファイル: {len(log_df)}件のログ")
    ...
    # Google Sheets 手動同期
    if spreadsheet:
        st.markdown("### Google Sheets 同期")
        
        col_sync1, col_sync2 = st.columns(2)
        
        with col_sync1:
            if st.button("🔄 Google Sheets から最新データを取得"):
                try:
                    fresh_data = load_data_from_sheets(spreadsheet)
                    st.success(f"✅ 最新データ取得完了: {len(fresh_data)}件")
                    st.rerun()
                except Exception as e:
                    st.error(f"❌ データ取得エラー: {e}")
        
        with col_sync2:
            if st.button("🧹 Google Sheets データをクリア", type="secondary"):
                try:
                    worksheet = spreadsheet.worksheet("TrainingLog")
                    worksheet.clear()
                    # ヘッダーを再設定（カテゴリー列を含む）
                    worksheet.append_row(["日付", "プログラム名", "名前", "エクササイズ名", "カテゴリー", "set", "負荷", "回数", "総負荷量"])
                    st.success("✅ Google Sheets データをクリアしました")
                    st.rerun()
                except Exception as e:
    ...
    if spreadsheet:
        st.info("""
        **バスケットボール トレーニングシステム v2.1 (Google Sheets連携版 + カテゴリー対応)**
        
        - ✅ Google Sheets での永続的データ保存
        - 🔄 リアルタイム同期
        - 📱 複数デバイス対応
        - 💾 自動バックアップ
        - 📊 クラウドベース分析
        ...
        **サポートファイル形式:** Excel (.xlsx, .xls), CSV
        ...
        st.warning("""
        **バスケットボール トレーニングシステム v1.1 (ローカル版 + カテゴリー対応)**
