        ...
        with st.expander(f"{program}", expanded=len(filtered_programs) <= 3):
            ...
            if len(main_exercises) > 0:
                ...
                def format_load(load):
                    load_str = str(load)
                    if load_str.replace('.', '').replace('-', '').isdigit() and load_str != '-':
                        try:
                            load_val = float(load_str)
                            if load_val <= 1.0 and load_val > 0:
                                return f"{load_val*100:.0f}%"
                        except:
                            pass
                    return load_str
                
                if '負荷' in display_df.columns:
                    display_df['負荷'] = display_df['負荷'].apply(format_load)
                
                # カテゴリー表示を整形
                if 'カテゴリー' in display_df.columns:
                    display_df['カテゴリー'] = display_df['カテゴリー'].apply(lambda x: x if x != '' and x != 'nan' and pd.notna(x) else '-')
