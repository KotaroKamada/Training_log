        ...
        for idx, exercise in enumerate(grouped_exercises):
            ...
            if st.session_state.selected_exercise_idx == idx:
                ...
                with st.expander(f"記録入力: {exercise_title}", expanded=True):
                    ...
                    # デバッグ情報表示
                    if st.checkbox("デバッグ情報を表示", key=f"debug_{idx}"):
                        st.write("**セッション状態のキー一覧:**")
                        debug_keys = [k for k in st.session_state.keys() if str(idx) in str(k)]
                        st.write(debug_keys)
                    
                    # 全適用フラグの初期化
                    apply_all_key = f"apply_all_trigger_{idx}"
                    
                    # デバッグ: フラグの状態を表示
                    if f"debug_{idx}" in st.session_state and st.session_state[f"debug_{idx}"]:
                        st.write(f"全適用フラグ ({apply_all_key}): {st.session_state.get(apply_all_key, 'なし')}")
                    ...
                    for set_num in range(actual_sets):
                        ...
                        # SET2以降で、全適用フラグが立っている場合はSET1の値を使用
                        use_set1_values = (set_num > 0 and 
                                          apply_all_key in st.session_state and 
                                          st.session_state[apply_all_key])
                        
                        # デバッグ情報
                        if f"debug_{idx}" in st.session_state and st.session_state[f"debug_{idx}"] and set_num > 0:
                            st.write(f"SET{set_num + 1}: use_set1_values = {use_set1_values}")
                        
                        with col1:
                            if use_set1_values and f"unit_{idx}_0" in st.session_state:
                                # SET1の単位を取得
                                unit_from_set1 = st.session_state[f"unit_{idx}_0"]
                                units_list = ["kg", "%", "体重", "その他"]
                                try:
                                    unit_default_idx = units_list.index(unit_from_set1)
                                except:
                                    unit_default_idx = 0
                            else:
                                unit_default_idx = 0
                            
                            unit = st.selectbox("単位", ["kg", "%", "体重", "その他"], 
                        ...
                        with col2:
                            if unit == "その他":
                                if use_set1_values and f"load_text_{idx}_0" in st.session_state:
                                    load_default = st.session_state[f"load_text_{idx}_0"]
                                else:
                                    load_default = ""
                                set_load = st.text_input("負荷", value=load_default, 
                                                        key=f"load_text_{idx}_{set_num}", 
                                                        placeholder="負荷", 
                                                        label_visibility="collapsed")
                            elif unit == "体重":
                                set_load = "体重"
                                st.text_input("負荷", value="体重", disabled=True, 
                                            key=f"load_disabled_{idx}_{set_num}", 
                                            label_visibility="collapsed")
                            else:
                                if use_set1_values and f"load_val_{idx}_0" in st.session_state:
                                    load_default = st.session_state[f"load_val_{idx}_0"]
                                else:
                                    load_default = 0.0
                                load_value = st.number_input("値", min_value=0.0, value=load_default, 
                        ...
                        with col3:
                            if use_set1_values and f"rep_{idx}_0" in st.session_state:
                                rep_default = st.session_state[f"rep_{idx}_0"]
                            else:
                                rep_default = 1
                            set_rep = st.number_input("レップ数", min_value=0, value=rep_default, 
                        ...
                        with col4:
                            if set_num == 0 and actual_sets > 1:
                                if st.button("全適用", key=f"copy_all_btn_{idx}", help="この設定を全セットに適用"):
                                    # デバッグ: ボタンが押されたことを確認
                                    st.write("🔴 全適用ボタンが押されました！")
                                    # フラグを立てて再描画
                                    st.session_state[apply_all_key] = True
                                    st.rerun()
                            else:
                                st.write("")
                    
                    # 全適用が実行された後はフラグをリセット
                    if apply_all_key in st.session_state and st.session_state[apply_all_key]:
                        st.session_state[apply_all_key] = False
                        ...
                        if st.button(f"{exercise['Exercise']} 完了", key=f"complete_{idx}", type="primary", use_container_width=True):
                            if not player_name:
                                ...
                                if saved_sets > 0:
                                    ...
                                    # セッション状態のクリーンアップ
                                    if apply_all_key in st.session_state:
                                        del st.session_state[apply_all_key]
                                    st.session_state.selected_exercise_idx = None
                                    st.rerun()
                    with col_btn2:
        ...
        if st.session_state.selected_exercise_idx is None:
            ...
            if st.button("全プログラム完了", type="primary", use_container_width=True):
                ...
                for key in list(st.session_state.keys()):
                    if key.startswith(('copy_data_', 'sets_', 'unit_', 'load_', 'rep_', 'comment_')):
