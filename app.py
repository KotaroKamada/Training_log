        ...
        for idx, exercise in enumerate(grouped_exercises):
            ...
            if st.session_state.selected_exercise_idx == idx:
                ...
                with st.expander(f"記録入力: {exercise_title}", expanded=True):
                    ...
                    # 全適用用のセッション状態を初期化
                    copy_key_base = f"copy_data_{idx}"
                    if copy_key_base not in st.session_state:
                        st.session_state[copy_key_base] = None
                    ...
                    for set_num in range(actual_sets):
                        ...
                        # 2セット目以降で全適用データがあれば使用
                        use_copied = set_num > 0 and st.session_state[copy_key_base] is not None
                        
                        with col1:
                            if use_copied:
                                unit_default = st.session_state[copy_key_base]['unit_index']
                            else:
                                unit_default = 0
                        ...
                        with col2:
                            if unit == "その他":
                                if use_copied and st.session_state[copy_key_base]['unit'] == "その他":
                                    load_default = st.session_state[copy_key_base]['load_text']
                                else:
                                    load_default = ""
                                set_load = st.text_input("負荷", value=load_default, key=f"load_{idx}_{set_num}", placeholder="負荷", label_visibility="collapsed")
                                if set_num == 0:
                                    set1_load_text = set_load
                            elif unit == "体重":
                                set_load = "体重"
                                ...
                                if use_copied and st.session_state[copy_key_base]['unit'] == unit:
                                    load_default = st.session_state[copy_key_base]['load_value']
                                else:
                                    load_default = 0.0
                                ...
                                if set_num == 0:
                                    set1_load_value = load_value
                            loads.append(set_load)
                        
                        with col3:
                            if use_copied:
                                rep_default = st.session_state[copy_key_base]['reps']
                            else:
                                rep_default = 1
                        ...
                        with col4:
                            if set_num == 0 and actual_sets > 1:
                                if st.button("全適用", key=f"copy_all_{idx}", help="この設定を全セットに適用"):
                                    ...
                                    copy_data = {
                                        'unit': set1_unit,
                                        'unit_index': units_list.index(set1_unit),
                                        'reps': set1_reps,
                                    }
                                    if set1_unit == "その他":
                                        copy_data['load_text'] = set1_load_text if set1_load_text else ""
                                        copy_data['load_value'] = 0.0
                                    elif set1_unit == "体重":
                                        copy_data['load_text'] = "体重"
                                        copy_data['load_value'] = 0.0
                                    else:
                                        copy_data['load_text'] = ""
                                        copy_data['load_value'] = set1_load_value if set1_load_value is not None else 0.0
                                    
                                    st.session_state[copy_key_base] = copy_data
                                    st.rerun()
                            else:
                                st.write("")
                        ...
                        if st.button(f"{exercise['Exercise']} 完了", key=f"complete_{idx}", type="primary", use_container_width=True):
                            if not player_name:
                                ...
                                if saved_sets > 0:
                                    ...
                                    # セッション状態のクリーンアップ
                                    if copy_key_base in st.session_state:
                                        del st.session_state[copy_key_base]
                                    st.session_state.selected_exercise_idx = None
                                    st.rerun()
                    with col_btn2:
