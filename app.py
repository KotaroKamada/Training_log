...
if st.sidebar.button("🔄 セッションをリセット"):
    # selected_type以外を削除
    keys_to_delete = [k for k in st.session_state.keys() if k != 'selected_type']
...
if st.sidebar.button("タイプ変更"):
    # selected_type のみリセット（他のセッション状態は保持しない）
    st.session_state.selected_type = None
    st.rerun()
...
if page == "Training Log 入力":
    ...
    if selected_program:
        ...
        if st.session_state.selected_exercise_idx is None:
            ...
            if st.button("全プログラム完了", type="primary", use_container_width=True):
                ...
                # selected_type以外のセッション状態をクリア
                keys_to_delete = [k for k in list(st.session_state.keys()) if k.startswith(('apply_trigger_', 'sets_', 'unit_', 'load_', 'rep_', 'comment_')) and k != 'selected_type']
                for key in keys_to_delete:
                    del st.session_state[key]
                st.rerun()
    ...
    st.markdown("---")
    st.markdown("### システム情報")
    st.info("""**バスケットボール トレーニングシステム v2.2 (バグ修正版)**
