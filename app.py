...
def load_data_from_sheets_cached(spreadsheet_id):
    try:
        if not spreadsheet:
            return pd.DataFrame(columns=["日付", "プログラム名", "名前", "エクササイズ名", "set", "負荷", "回数", "総負荷量"])
        ...
        # シートが存在しない場合は作成
        try:
            worksheet = spreadsheet.add_worksheet(title="TrainingLog", rows="1000", cols="8")
            # ヘッダーを設定
            worksheet.append_row(["日付", "プログラム名", "名前", "エクササイズ名", "set", "負荷", "回数", "総負荷量"])
        except:
            pass
        return pd.DataFrame(columns=["日付", "プログラム名", "名前", "エクササイズ名", "set", "負荷", "回数", "総負荷量"])
    except Exception as e:
...
{exercise_details}"""
    ...
    if spreadsheet:
        st.info("""
        **バスケットボール トレーニングシステム v2.0 (Google Sheets連携版)**
        ...
        **サポートファイル形式:** Excel (.xlsx, .xls), CSV
        ...
        st.warning("""
        **バスケットボール トレーニングシステム v1.0 (ローカル版)**
