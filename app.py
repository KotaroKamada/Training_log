...
{exercise_details}"""
    ...
    with col3:
        # エクササイズ選択
        if 'エクササイズ名' in log_df.columns:
            available_exercises = ["すべて"] + sorted(log_df['エクササイズ名'].unique().tolist())
            selected_exercise = st.selectbox("エクササイズ", available_exercises)
        else:
            selected_exercise = "すべて"
        # 日付範囲選択
    ...
    # エクササイズでフィルタ
    if selected_exercise != "すべて" and 'エクササイズ名' in filtered_df.columns:
        filtered_df = filtered_df[filtered_df['エクササイズ名'] == selected_exercise]
