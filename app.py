        ...
        for idx, exercise in enumerate(grouped_exercises):
            ...
            if st.session_state.selected_exercise_idx == idx:
                ...
                with st.expander(f"記録入力: {exercise_title}", expanded=True):
                    ...
                    # フラグが立っているかチェック（ループ前に一度だけ）
                    should_apply_all = st.session_state.get(apply_all_key, False)
                    ...
                    for set_num in range(actual_sets):
                        ...
                        use_set1_values = set_num > 0 and should_apply_all
                        
                        # デバッグ情報
                        if f"debug_{idx}" in st.session_state and st.session_state[f"debug_{idx}"] and set_num > 0:
                            st.write(f"SET{set_num + 1}: use_set1_values = {use_set1_values}, should_apply_all = {should_apply_all}")
